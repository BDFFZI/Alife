...
function updateResConvertButton(g) {
  ...
  if (baseW > 0 && baseH > 0) {
    if (baseW === screenSize.width && baseH === screenSize.height) {
      if (btn) btn.style.display = 'none';
      if (stateEl) { stateEl.textContent = '一致'; stateEl.className = 'res-state ok'; }
    } else {
      if (btn) { btn.style.display = ''; btn.textContent = `转换分辨率 ${baseW}×${baseH} → ${screenSize.width}×${screenSize.height}`; btn.title = '按比例缩放所有采集区域到当前分辨率，并更新基准分辨率'; }
      if (stateEl) { stateEl.textContent = '不一致'; stateEl.className = 'res-state bad'; }
    }
  } else {
    // 基准未记录（老配置）：以当前分辨率为基准记录，方便日后分辨率变化时转换
    if (btn) { btn.style.display = ''; btn.textContent = `记录基准分辨率 ${screenSize.width}×${screenSize.height}`; btn.title = '本游戏尚未记录基准分辨率，点击以当前分辨率作为基准（后续分辨率变化时可一键转换）'; }
...
function renderTargetList(g) {
  ...
  g.Collectors.forEach((t, i) => {
    ...
    COLLECTOR_TYPES.forEach((ct) => {
      ...
      const firstType = (COLLECTOR_TYPES[0] && COLLECTOR_TYPES[0].type) || 'text';
      o.selected = (t.Sampler || firstType) === ct.type;
...
// 按比例转换当前游戏的所有采集区域到当前物理分辨率，并更新基准分辨率。
// 基准未记录(0)时仅记录当前分辨率为基准（scale=1，不缩放区域）。
function convertResolution() {
  ...
  if (baseW > 0 && baseH > 0 && (baseW !== screenSize.width || baseH !== screenSize.height)) {
    const sx = screenSize.width / baseW;
    const sy = screenSize.height / baseH;
  ...
  // 更新基准分辨率并落盘
  g.BaseScreenWidth = screenSize.width;
  g.BaseScreenHeight = screenSize.height;
  saveConfigNow();
  renderGameEditor();
}
