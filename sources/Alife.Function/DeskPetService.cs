...
public class DeskPetService : Plugin, IAsyncDisposable
{
    ...
    public override Task StartAsync(Kernel kernel, ChatActivity chatActivity)
    {
        ...
        try
        {
            string petExePath = $"{PathEnvironment.OutputPath}/Alife.DeskPet/debug/Alife.DeskPet.exe";
            if (File.Exists(petExePath) == false)
            {
                Console.WriteLine(Path.GetFullPath(petExePath));
                Console.WriteLine($"[DeskPetService] Error: Could not find Pet EXE at {petExePath}");
                return Task.CompletedTask;
            }
            ...
            Console.WriteLine("[DeskPetService] Pet process started successfully.");
        }
        catch (Exception ex)
        {
    ...
    void SendToPet(object msg)
    {
        if (petProcess is { HasExited: false })
        {
            ...
            {
                Console.WriteLine($"[DeskPetService] Send Error: {ex.Message}");
            }
        }
    }
