    ...
    IConfigurable<GameCompanionConfig>
{
        ...
        lock (monitorLock)
        {
            ...
            monitor.BeforeSample = async () => {
                if (HasColorCollector(game) && !(editorController?.IsOverlayEditing() ?? false))
                {
                    ...
                    await Task.Delay(50); // 等待合成器移除覆盖层，避免取样到覆盖层
                }
                return true;
            };
            monitor.AfterSample = () => {
                editorController?.RestoreOverlayForSample();
                return Task.CompletedTask;
            };
    ...
    protected override async Task OnAwake()
    {
        ...
        ScreenFrame.EnsureProcessDpiAware();
        ...
        // 浮窗作为唯一入口：模块启动后始终显示
        await editorController.ShowFloatAsync();
        ...
        messageFilterService.AddMessageReplyRule(new MessageReplyRule {
            ...
            CorrectionMessage = () =>
                "陪玩数据/评论请用 <speak>标签</speak> 回复。若不想说话，请发送空的 <speak></speak> 标签。"
        }, DestroyCancellationToken);

        var handler = new XmlHandler(this) {
            Description = "此功能可为你开启额外的数据感知，让你拥有参与部分游戏陪玩的能力",
            Explanation = """
                          开启陪玩后系统将在游戏数据变动时，适时的向你推送变动的游戏数据。
                          请将他们视为玩家实时状态，输出符合当下场景的陪玩台词，让玩家感到你真的在「看着」他玩。
                          保持真人观战口吻，简短自然地插话评论，避免流水账式的一致报数据。
                          若要跳过评论可以发送空标签（<speak></speak>）。
                          """
        };
    ...
    static bool HasColorCollector(GameConfig game)
        => game.Collectors.Any(c => c.IsEnable && c is PixelCollectorConfig);
    ...
    void OnPluginAlcUnloading(System.Runtime.Loader.AssemblyLoadContext context)
    {
        ...
        {
            Console.WriteLine($"[Companion] 卸载时停止监控异常: {e.Message}");
        }
        try
        {
            editorController?.Dispose();
        ...
        {
            Console.WriteLine($"[Companion] 卸载时释放编辑器异常: {e.Message}");
        }
    }
}