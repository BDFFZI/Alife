...
public class VisionAnalyzer : IDisposable
{
    ...
    public async Task InitAsync(string? scriptPath = null, string? modelPath = null, int timeoutSeconds = 120, Action<string>? onLog = null)
    {
        ...
        try
        {
            while (!cts.Token.IsCancellationRequested)
            {
                ...
                if (line == "READY") { _ready = true; return; }
                // 可能是错误信息
                if (line.StartsWith("{"))
                {
    ...
    /// 图像描述：用中文描述图片内容。
    /// </summary>
    public Task<string> CaptionAsync(string imagePath, CancellationToken ct = default)
        => SendRequestAsync(new { action = "caption", image_path = imagePath }, ct);

    /// <summary>
    /// 视觉问答：用中文提问，获得中文回答。
    /// </summary>
    public Task<string> QueryAsync(string imagePath, string question, CancellationToken ct = default)
        => SendRequestAsync(new { action = "query", image_path = imagePath, question }, ct);
